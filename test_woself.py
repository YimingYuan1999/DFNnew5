...
parser.add_argument("--logdir", type=str, default="logs/dense_gelu/b2t7_kc_H", help='path to model and log files')
parser.add_argument("--data_path", type=str, default="/1T/rain_removal/PReNet-master/datasets/test/try", help='path to training data')
parser.add_argument("--save_path", type=str, default="./release/results/dense_gelu/plot/rain_steak_back", help='path to save results')
...
def main():
    ...
    # Build model
    print('Loading model ...\n')
    model = RRFBN(opt.recurrent_iter, opt.use_GPU)
    print_network(model)
    if opt.use_GPU:
        model = model.cuda()
