...
parser.add_argument("--logdir", type=str, default="logs/final/b2t1_Light", help='path to model and log files')
parser.add_argument("--data_path", type=str, default="/1T/rain_removal/PReNet-master/datasets/real_test_1000/rain", help='path to training data')
parser.add_argument("--save_path", type=str, default="./release/results/final/b2t1_Light_real", help='path to save results')
...
def main():
    ...
    # Build model
    print('Loading model ...\n')
    model = RRFBN(opt.recurrent_iter)
    print_network(model)
    if opt.use_GPU:
        model = model.cuda()
    model.load_state_dict(torch.load(os.path.join(opt.logdir, 'net_latest.pth')))
    #model.load_state_dict(torch.load(os.path.join(opt.logdir, 'net_epoch70.pth')))
    model.eval()
